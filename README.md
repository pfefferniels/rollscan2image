...
02-07`. On 12 February 2001 he offered the design on the Mechanical Music
...
is read as a stepper at 200 dpi, following `CISREPORT`, which says
"it is assumed to be a Stepper Scanner and the Horizontal Resolution is inferred
from the Scan Width", and PlaySK, which defaults an unreadable type to 200 dpi.
...
No early file was available, but its header is attested. Knobloch prints the
...
that follow adding to the line width. The rest, the resolution and the scanner
type, is still assumption.
...
- `CIS33.BAS`, `FIXCIS.BAS` and the rest of the Mk3a scanner source, QuickBASIC,
  ...
  <https://web.archive.org/web/2010id_/http://www.iammp.org/design/files/MK3a_software.ZIP>.
...
- Stahnke, W. *View and Edit Music Roll Image*, V10.48, 1991–1997, with sample
  `.ann`, `.bar` and `.web` files, in `mmdigest.com/Tech/57504A.zip` (recovered
  from the Wayback Machine; gone from the live site). The `.ANN` sidecar that
  `cis2image.py` reads is Stahnke's, not the scanner group's: his own reader
  carries the whole vocabulary, 37 keys from `/title:` to `/sample_rate:`, and
  his sample annotation is one `/key: value` per line. Trachtman adopted the
  scheme for his MIDI text events and says so.
- Stibbons, R. *How to build a Piano Roll Scanner*, signed 4 February 2001,
  archived at
  <https://web.archive.org/web/2010id_/http://www.iammp.org/design/files/How_to_build_a_Roll_Scanner.doc>
